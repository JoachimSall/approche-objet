...
public class VillesPeupleesRegion extends MenuService {
	...
	public void traiter(List<Ville> villes, Scanner scanner) {
		...
		int compteur = 0;
		HashMap<String, Integer> villeRegionPop = new HashMap<>();
		for(Ville ville : villes) {
			Integer population = ville.getPop();
			String nomVille = ville.getNom();
			if (ville.getCodeRegion().equals(scan) || ville.getRegion().toUpperCase().equals(scan.toUpperCase())) {
				villeRegionPop.put(nomVille, population);
				compteur++;
			}
		}
		...
		if (compteur == 0) {
			...
			List<Map.Entry<String, Integer>> listVilles = new LinkedList<Map.Entry<String, Integer>>(villeRegionPop.entrySet());
